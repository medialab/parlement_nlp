...
from lib.utils import fuzz_word
...
class Dossiers:
    def _actes_legis_rec(self, obj, attribute=[]):
        ...
        for child in children:
            for attr in attribute:
                if attr in child and isinstance(child, dict):
                    # results.append(child[attr])
                    if "dateActe" in child:
                        results.append(
                            {"id": child[attr], "date": child["dateActe"][:10]}
                        )
    ...
    def __init__(self, dossiers_dir):
        ...
        for path in glob.glob(join(dossiers_dir, "*.json")):
            ...
            title = ebbe.getpath(
                data, ["dossierParlementaire", "titreDossier", "titre"]
            )
            label = ebbe.getpath(
                data, ["dossierParlementaire", "procedureParlementaire", "libelle"]
            )
            debates_id = self._actes_legis_rec(
                ebbe.get(data, "dossierParlementaire"), ["reunionRef"]
            )
            bills = self._actes_legis_rec(
                ebbe.get(data, "dossierParlementaire"), ["texteAssocie"]
            )
    ...
    def from_seance(self, id, clue=None):
        ...
        prox = Counter({t: fuzz_word(clue, t) for t in titles})
        try:
