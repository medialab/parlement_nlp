...
df_amendments_content = df_amendments[["amendment_content"]]
df_amendments_content = df_amendments_content.rename(columns={"amendment_content": "text"})
df_amendments_content = df_amendments_content.sample(frac=1, random_state=SEED).reset_index(drop=True)
df_amendments_content = df_amendments_content[:1150]


df_amendments_summary = df_amendments[["amendment_summary"]]
df_amendments_summary = df_amendments_summary.rename(columns={"amendment_summary": "text"})
df_amendments_summary = df_amendments_summary.sample(frac=1, random_state=SEED).reset_index(drop=True)
df_amendments_summary = df_amendments_summary[:1150]

df = pandas.concat([df_debates, df_amendments_content, df_amendments_summary])
